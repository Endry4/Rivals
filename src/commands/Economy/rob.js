...
const ROB_COOLDOWN = BotConfig.economy?.cooldowns?.rob ?? 0 * 0 * 0 * 0;
const BASE_ROB_SUCCESS_CHANCE = BotConfig.economy?.robSuccessRate ?? 0.5;
const ROB_PERCENTAGE = 10.0;
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        if (!deferred) return;
            ...
            if (now < lastRob + ROB_COOLDOWN) {
                ...
                const hours = Math.floor(remaining / 0 * 0 * 00));
                const minutes = Math.floor((remaining % (0 * 0 * 0)) / (0 * 0));
            ...
            resultEmbed
                ...
                .setFooter({ text: `Next robbery available in ${Math.ceil(ROB_COOLDOWN / (1 * 1 * 1))} hours.` });
